      ...
      <div class="container-fluid page-body-wrapper">
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            ...
            $query = mysqli_query($connect_db,"SELECT * FROM `other_assets` WHERE `assetID` = $assetID")

            or die( mysqli_error($connect_db));
            while ($row = mysqli_fetch_array($query)) {

                $output = "";
               echo $ammo_name = '[ '.ucwords($row['asset_serial_no']).' '.ucwords($row['asset_name']).' ]';
              ...
              <p class="card-description"><a href="booking"><code><i class="mdi mdi-plus f-22 text-green"></i><i class="mdi mdi-book f-22 text-green" 
                ...
                <table id="administrators-list" class="table table-bordered ">
                  <thead>
                     ...
                     <?php
                          $assetID=$_GET['assetID'];
                          $query = mysqli_query($connect_db,"SELECT * FROM `asset_bookings` WHERE `assetID` = $assetID ORDER BY `bookAssetID` ")
                          ...
                          while ($row = mysqli_fetch_array($query)) {
                              ...
                              <td>'.$row['datetime'].'</td>                 
                              <td>
                              <a href="#ammo-booking-details-'.$row['bookAssetID'].'" data-toggle="modal" style="text-decoration:none;color:#fff;">
                             <img src="assets/images/officer_images/'.$row['officer_image'].'" alt=""> &nbsp; '.$row['to_officer'].'
                              </a> 
                              </td>
                              <td>
                              <a href="#return-ammo-officer-'.$row['bookAssetID'].'"  
                              ...
                              <td> 
                              <a href="update-ammo-booking?firearm-booking-ticket='.$row['bookAssetID'].'><i class="mdi mdi-playlist-edit f-16 mr-15 text-green"></i></a>
                              &nbsp; &nbsp;<a href="#delete-ammo-booking-'.$row['bookAssetID'].'" data-toggle="modal"><i class="mdi mdi-delete f-16 mr-15 text-red"></i></a>
                              </td>
                          ...
                          <a href="#" class=" btn-outline-success" style="padding-top:6px;text-decoration:none;">[ '.$row['asset_returns'].' ]</a>   
                            ...
                            <td> '.$row['armourer_issuer'].' </td>
                          <td> 
                          <a href="update-ammo-booking?firearm-booking-ticket='.$row['bookAssetID'].'">
                          <i class="mdi mdi-playlist-edit f-16 mr-15 text-green"></i></a>
                          &nbsp; &nbsp;
                          <a href="#delete-ammo-booking-'.$row['bookAssetID'].'" data-toggle="modal">
                          <i class="mdi mdi-delete f-16 mr-15 text-red"></i></a>
                          </td>
                        </tr>
                          ';
                        }
                        include('actions-officer-asset.php');
                        include('actions.php');
