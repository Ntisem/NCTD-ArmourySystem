...
<html lang="en">
  <head>
    ...
    <title>GPS ARMOURY SYSTEM - FIREARM BOOKING HISTORY</title>
    <!-- plugins:css -->
      ...
      <div class="container-fluid page-body-wrapper">
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            ...
            $query = mysqli_query($connect_db,"SELECT * FROM `firearms` WHERE `firearmID` = $firearmID")

            or die( mysqli_error($connect_db));
            while ($row = mysqli_fetch_array($query)) {

                $output = "";
               echo $firearm_name = '[ '.ucwords($row['firearm_serial_no']).' '.ucwords($row['firearm_name']).' ]';
              ...
              <p class="card-description"><a href="booking"><code><i class="mdi mdi-plus f-22 text-green"></i><i class="mdi mdi-book f-22 text-green" 
                ...
                <table id="administrators-list" class="table table-bordered ">
                  <thead>
                     ...
                     <?php
                          $firearmID=$_GET['firearmID'];
                          $query = mysqli_query($connect_db,"SELECT * FROM `bookings` WHERE `firearmID` = $firearmID ORDER BY `bookingID` ")
                        ...
                        include('actions-officer-firearm.php');
                        include('actions.php');
